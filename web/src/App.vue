...
type Stop = { id: string; sequence: number; kind?: string; title: string; address?: string; location?: LocationData; time_window?: { arrival?: string; departure?: string }; description_markdown?: string; links?: LinkData[]; weather?: Record<string, unknown> }
...
declare global { interface Window { BMap?: any; BMapGL?: any } }
...
function pointFor(stop: Stop): (Coord & { crs: string }) | null {
...
function selectStop(stop: Stop) { selectedStopId.value = stop.id; if (window.matchMedia('(max-width: 900px)').matches) { panelOpen.value = false; localStorage.setItem('journeyin.panelOpen', 'false') }; if (mapInstance && mapAPI) { const point = pointFor(stop); if (point) mapInstance.panTo(new mapAPI.Point(point.lng, point.lat)) } }
...
async function searchPlaces() {
  ...
  try {
    const response = await apiFetch('/api/v1/maps/pois/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ provider: 'baidu', query: searchQuery.value.trim(), region: searchRegion.value.trim(), page: 1, page_size: 10 }) })
...
async function addPlaceToTrip(candidate: PlaceCandidate) {
  ...
  try {
    ...
    applyTripPayload(payload); panelMode.value = 'journey'; searchMessage.value = '已添加“' + candidate.name + '”，路线尚未生成'; searchResults.value = []
...
async function openNavigation(provider: 'baidu' | 'amap') {
  const stop = selectedStop.value; const point = stop && pointFor(stop)
...
function weatherText(stop: Stop) { const weather = stop.weather || {}; const condition = weather.condition || weather.text_day || weather.text || '暂无天气快照'; const temperature = weather.temperature_c ?? weather.temp; return temperature === undefined ? String(condition) : String(condition) + ' · ' + String(temperature) + '°C' }
function refresh(event: CustomEvent) { loadTrips().finally(() => event.detail.complete()) }
function closeDetail() { selectedStopId.value = '' }
...
<template>
  <IonApp><IonPage class="journey-page">
    <IonHeader translucent><IonToolbar><IonTitle><span class="brand-dot"></span>JourneyIn</IonTitle><div slot="end" class="header-actions"><IonButton fill="clear" aria-label="显示或隐藏行程面板" @click="togglePanel"><IonIcon :icon="menuOutline" /></IonButton><IonButton fill="clear" aria-label="打开设置" @click="openSettings()"><IonIcon :icon="settingsOutline" /></IonButton><IonButton fill="clear" aria-label="新建旅行规划" @click="newTripOpen = true"><IonIcon :icon="addOutline" /></IonButton></div></IonToolbar></IonHeader>
    <IonContent :fullscreen="true"><IonRefresher slot="fixed" @ionRefresh="refresh"><IonRefresherContent /></IonRefresher>
      <main class="map-shell">
        <div class="map-canvas"><div v-if="keyConfigured && tripDocument && !mapError" ref="mapContainer" id="bmap"></div><div v-if="!tripDocument || !keyConfigured || mapError" class="map-fallback"><IonIcon :icon="mapOutline"/><strong>{{ !tripDocument ? '选择或创建一条旅行规划' : mapError || '百度地图未配置' }}</strong><span>{{ !tripDocument ? '创建或导入 Trip 后显示地图。' : mapError ? '请确认浏览器端 AK、JSAPI 服务、域名白名单和网络连接。当前页面：' + serverURL : '配置浏览器端 Key 后显示真实地图。当前不会伪造道路或路线。' }}</span><IonChip color="warning"><IonIcon :icon="cloudOfflineOutline"/> {{ !tripDocument ? '等待行程' : '降级模式' }}</IonChip></div><div v-if="keyConfigured && tripDocument && !mapError && !mapReady" class="map-loading"><IonIcon :icon="mapOutline"/><span>正在加载百度地图…</span></div></div>
        <div class="map-hud"><button v-if="!panelOpen" class="panel-open-button" aria-label="显示行程面板" @click="togglePanel"><IonIcon :icon="menuOutline"/> 行程面板</button><div class="map-status-pill"><IonIcon :icon="keyConfigured && mapReady && !mapError ? sunnyOutline : cloudOfflineOutline"/> {{ !tripDocument ? '等待行程' : !keyConfigured ? '离线数据可用' : mapError ? '百度地图不可用' : mapReady ? '百度地图已连接' : '百度地图加载中' }} · {{ visibleStops.length }} 个规划点</div></div>
        <section v-if="error || shareURL" class="map-notices"><div v-if="error" class="global-error">{{ error }}<button aria-label="关闭错误" @click="error = ''">×</button></div><div v-if="shareURL" class="share-banner"><span>只读分享：<a :href="shareURL" target="_blank" rel="noopener noreferrer">{{ shareURL }}</a></span><button aria-label="关闭分享提示" @click="shareURL = ''">×</button></div></section>
        <aside v-if="panelOpen" class="floating-panel plan-panel" aria-label="行程规划面板"><div class="panel-head"><div><p class="eyebrow">JOURNEYIN</p><h2>{{ selected?.title || '旅行规划' }}</h2><p class="panel-subtitle">{{ selected ? selected.start_date + ' — ' + selected.end_date : '本地行程工作区' }}</p></div><button class="panel-close" aria-label="隐藏行程面板" @click="togglePanel">×</button></div><div class="panel-tabs"><button :class="{ selected: panelMode === 'journey' }" @click="panelMode = 'journey'">行程</button><button :class="{ selected: panelMode === 'search' }" :disabled="!selected" @click="panelMode = 'search'"><IonIcon :icon="searchOutline"/> 添加地点</button></div><div class="panel-scroll">
          <template v-if="panelMode === 'journey'"><div class="section-heading"><div><p class="eyebrow">YOUR JOURNEYS</p><h3>旅行规划</h3></div><IonBadge color="primary">{{ trips.length }}</IonBadge></div><p v-if="loading" class="muted">正在加载…</p><div v-else-if="!trips.length" class="empty"><p>还没有旅行规划。</p><IonButton size="small" @click="newTripOpen = true"><IonIcon slot="start" :icon="addOutline"/> 新建规划</IonButton><p class="empty-hint">也可以导入已有 Trip JSON。</p></div><div v-else class="trip-cards"><button v-for="trip in trips" :key="trip.id" class="trip-card" :class="{ active: selected?.id === trip.id }" @click="loadDetail(trip)"><span><b>{{ trip.title }}</b><small>{{ trip.start_date }} — {{ trip.end_date }}</small><small>{{ trip.days ?? '—' }} 天 · {{ trip.stops ?? '—' }} 个规划点</small></span><IonChip color="light">v{{ trip.revision }}</IonChip></button></div><template v-if="selected && tripDocument"><div class="panel-section"><div class="section-heading compact"><div><p class="eyebrow">ITINERARY</p><h3>规划点</h3></div><span class="count-label">{{ visibleStops.length }}</span></div><div class="day-tabs"><button :class="{ selected: selectedDay === 'all' }" @click="selectedDay = 'all'">全程</button><button v-for="(_, index) in tripDocument.days" :key="index" :class="{ selected: selectedDay === index + 1 }" @click="selectedDay = index + 1">D{{ index + 1 }}</button></div><div class="plan-controls"><label>路线方式<select v-model="planningMode"><option value="walking">步行</option><option value="driving">驾车</option><option value="cycling">骑行</option><option value="transit">公交</option></select></label><IonButton size="small" :disabled="planningLoading || !plannableDays.length" @click="planRoutes"><IonIcon slot="start" :icon="navigateOutline"/> {{ planningLoading ? '规划中…' : '生成路线' }}</IonButton></div><p v-if="!plannableDays.length" class="hint">同一天添加至少两个地点后，点击“生成路线”。</p><p v-else-if="visibleDays.some(day => day.legs?.some(leg => leg.snapshots?.length))" class="route-ready"><IonIcon :icon="navigateOutline"/> 已有路线快照；重复点击会优先使用缓存。</p><div v-if="visibleStops.length" class="stop-list"><button v-for="stop in visibleStops" :key="stop.id" class="stop-row" :class="{ selected: selectedStopId === stop.id }" @click="selectStop(stop)"><span class="stop-number">{{ stop.sequence }}</span><span><b>{{ stop.title }}</b><small>{{ stop.address || '地址已保存，未填写显示名' }}</small></span><span class="stop-arrow">›</span></button></div><p v-else class="empty compact-empty">当前日期还没有规划点。</p><button class="add-place-cta" @click="panelMode = 'search'"><IonIcon :icon="searchOutline"/> 搜索并添加规划点</button></div></template></template>
          <template v-else><div class="section-heading"><div><p class="eyebrow">PLAN A STOP</p><h3>搜索地点</h3></div><button class="panel-back" @click="panelMode = 'journey'">返回行程</button></div><form class="search-form" @submit.prevent="searchPlaces"><label>地点或关键词<input v-model="searchQuery" placeholder="例如：西湖、咖啡馆、酒店" autocomplete="off" /></label><label>城市/区域（可选）<input v-model="searchRegion" placeholder="例如：杭州市" autocomplete="address-level2" /></label><IonButton type="submit" expand="block" :disabled="searchLoading"><IonIcon slot="start" :icon="searchOutline"/> {{ searchLoading ? '搜索中…' : '搜索地点' }}</IonButton></form><p class="search-help">只在点击搜索时调用百度 POI 接口。选择结果后会保存名称、地址、坐标、CRS 和百度 UID，之后绘制地图不重复查询。</p><p v-if="searchMessage" class="inline-message">{{ searchMessage }}</p><div class="search-results"><article v-for="(result, index) in searchResults" :key="result.id || result.name + index" class="search-result"><div><h4>{{ result.name }}</h4><p>{{ result.address || '地址待补充' }}</p><small v-if="result.location">BD-09LL · {{ result.location.lat.toFixed(6) }}, {{ result.location.lng.toFixed(6) }}</small></div><IonButton size="small" @click="addPlaceToTrip(result)">添加</IonButton></article></div></template>
        </div></aside>
        <aside v-if="selectedStop" class="details-drawer"><button class="close-button" aria-label="关闭详情" @click="closeDetail"><IonIcon :icon="closeOutline" /></button><p class="eyebrow">STOP {{ selectedStop.sequence }}</p><h2>{{ selectedStop.title }}</h2><p class="address">{{ selectedStop.address || '地址待解析' }}</p><div class="saved-location"><span>坐标已保存</span><small>{{ pointFor(selectedStop)?.crs || '未知 CRS' }} · {{ pointFor(selectedStop)?.lat.toFixed(6) }}, {{ pointFor(selectedStop)?.lng.toFixed(6) }}</small></div><div class="weather"><IonIcon :icon="sunnyOutline"/><span>{{ weatherText(selectedStop) }}</span></div><div v-if="selectedStop.description_markdown" class="markdown" v-html="renderMarkdown(selectedStop.description_markdown)"></div><p v-else class="muted">暂无地点说明。</p><div class="links" v-if="selectedStop.links?.length"><a v-for="link in selectedStop.links" :key="link.id || link.url" :href="safeURL(link.url)" target="_blank" rel="noopener noreferrer"><IonIcon :icon="linkOutline"/> {{ link.title }}</a></div><div class="nav-actions"><IonButton size="small" @click="openNavigation('baidu')"><IonIcon slot="start" :icon="navigateOutline"/> 百度导航</IonButton><IonButton size="small" fill="outline" @click="openNavigation('amap')"><IonIcon slot="start" :icon="navigateOutline"/> 高德导航</IonButton></div></aside>
      </main>
    </IonContent>
